        ...
        <main>
            ...
            <form action="" method="get">
                <fieldset>
                    ...
                    <?php
                    $string = "";
                    $string .= "<label for=\"$colunas[0]\">$colunas[0]:</label>\n\t\t\t\t\t";
                    $string .= "<img src=\"{$ufs[$uf][0]}\" alt=\"Bandeira do/de {$ufs[$uf][1]}\" id=\"$colunas[0]\"><br>\n\t\t\t\t\t";
                    for ($i=1;$i<4;$i++) {
                        $string .= "<label for=\"$colunas[$i]\">$colunas[$i]:</label>\n\t\t\t\t\t";
                        $string .= "<input type=\"text\" id=\"$colunas[$i]\" value=\"{$ufs[$uf][$i]}\"><br>\n\t\t\t\t\t";
                    }
                    for ($i=4;$i<14;$i++){
                        $string .= "<label for=\"$colunas[$i]\">$colunas[$i]:</label>\n\t\t\t\t\t";
                        $text = preg_replace("/[0-9,]/", "", $ufs[$uf][$i]);
                        if ($text=="") {
                            $string .= "<input type=\"text\" id=\"$colunas[$i]\" value=\"{$ufs[$uf][$i]}\"><br>\n\t\t\t\t\t";
                        }
                        else {
                            $value = str_replace(',', '.', $ufs[$uf][$i]) + 0;
                            $value = str_replace('.', ',', $value);
                            $string .= "<input type=\"text\" id=\"$colunas[$i]\" value=\"$value\">";
                            $string .= "<label for=\"$colunas[$i]\">$text</label><br>\n\t\t\t\t\t";
                        }
                    }
                    echo $string;
                    ?>
